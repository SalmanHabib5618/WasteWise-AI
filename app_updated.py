...
with tab1:
    ...
    with col2:
        ...
        if res:
            ...
            st.markdown(
                f'<div class="card"><b>Detected:</b> {esc(res["object"])}<br>'
                f'<b>Material:</b> {esc(res["material"])}</div>',
                unsafe_allow_html=True,
            )
            st.markdown(
                f'<div class="card"><b>Estimated Scrap Value:</b> {esc(res["value"])}<br>'
...
with tab2:
    ...
    if lat is not None and lon is not None:
        ...
        folium.Marker(
            [lat, lon],
            popup=f"User: {esc(current_user['username'])}",
